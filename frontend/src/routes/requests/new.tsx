...
export const Route = createFileRoute("/requests/new")({
  head: () => ({
    meta: [{ title: "Request a Block — Railway AI Block Planner" }],
  }),
  ...
  return (
    ...
    >
      <div className="flex min-w-0 flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <span className="min-w-0 break-words font-display text-base font-semibold text-cream sm:text-lg">
          {option.windowLabel}
        </span>

        <div className="shrink-0 self-start">
          <Tag tone={option.forcesDelay || option.overCapacity ? "danger" : "steel"}>
            {cadenceLabel[option.cadence] ?? option.cadence}
          </Tag>
        </div>
      </div>

      <div className="mt-2 break-words text-[11px] leading-5 text-steel">{option.note}</div>

      <div className="mt-1 break-words text-[10px] leading-5 text-steel">{option.trafficNote}</div>

      <div className="mt-3 grid grid-cols-2 gap-2 text-[10px] text-steel sm:flex sm:flex-wrap sm:gap-3">
        <span>
      ...
      {option.forcesDelay ? (
        <div className="mt-3 flex min-w-0 items-start gap-2 rounded border border-danger/40 bg-danger/10 p-2 text-[10px] leading-5 text-danger">
      ...
      {option.overCapacity ? (
        <div className="mt-3 flex min-w-0 items-start gap-2 rounded border border-danger/40 bg-danger/10 p-2 text-[10px] leading-5 text-danger">
          ...
          <span className="min-w-0 break-words">
            Needs more night windows than a month holds. Consider splitting the km span.
          </span>
        </div>
      ) : null}
    </button>
  ...
  return (
    <div className="min-w-0 space-y-2">
      {shown.map((s) => (
        <div
          key={s.blockId}
          className="grid min-w-0 gap-1.5 rounded border border-line bg-ink3/40 px-3 py-2 text-[11px] sm:grid-cols-[auto_1fr_auto_auto] sm:items-center sm:gap-3"
        >
          <span className="break-words text-signal">{s.blockId}</span>

          <span className="break-words text-cream sm:text-left">
...
function NewRequest() {
  ...
  const onConfirm = async () => {
    ...
    toast.success(
      `${ref} accepted — ${chosen.sessions.length} block${
        chosen.sessions.length > 1 ? "s" : ""
      } allotted`,
    );
  ...
  if (step === "form") {
    return (
      <form onSubmit={onSubmitToAi} className="min-w-0 space-y-4 sm:space-y-6" noValidate>
        <PageHeader
        ...
        <Panel title="Resources">
          ...
          <div className="mt-4 min-w-0">
            ...
            <div className="mt-2 grid min-w-0 grid-cols-1 gap-2 sm:flex sm:flex-wrap sm:gap-x-5 sm:gap-y-2">
          ...
          <div className="mt-5 flex min-w-0 flex-col gap-3 border-t border-line pt-5 sm:flex-row sm:flex-wrap sm:items-center">
            <ChromeButton
              type="submit"
              disabled={planning}
            ...
            {Object.keys(errors).length ? (
              <span className="break-words text-[11px] text-danger">
  ...
  if (step === "options" && plan) {
    return (
      <div className="min-w-0 space-y-4 sm:space-y-6">
        ...
        >
          {planError ? (
            <p className="mb-3 break-words rounded border border-danger/40 bg-danger/10 p-2 text-[11px] leading-5 text-danger">
              {planError}
            </p>
          ) : null}

          <p className="mb-4 text-xs leading-5 text-steel">
        ...
        <div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:flex-wrap">
          ...
          <GhostButton onClick={() => setStep("form")} className="w-full sm:w-auto">
            Edit request
          </GhostButton>
        </div>
      </div>
    ...
    return (
      <div className="min-w-0 space-y-4 sm:space-y-6">
          ...
          <Panel title="Submitted preferences">
            <dl className="space-y-3 text-[11px]">
              {[
                ...
                <div
                  key={k}
                  className="grid min-w-0 grid-cols-1 gap-1 border-b border-line/40 pb-2 sm:grid-cols-[8rem_minmax(0,1fr)] sm:gap-4"
                >
                  <dt className="text-steel">{k}</dt>

                  <dd className="min-w-0 break-words text-left text-cream sm:text-right">{v}</dd>
                </div>
              ))}
            </dl>
          </Panel>
          ...
          <div className="min-w-0 space-y-4 sm:space-y-6">
            <Panel title="Allotted window">
              <div className="min-w-0 space-y-3">
                <div className="flex min-w-0 items-start gap-2 font-display text-xl font-semibold text-cream sm:text-2xl">
                  <Clock className="mt-1 size-5 shrink-0 text-signal" />

                  <span className="min-w-0 break-words">{chosen.windowLabel}</span>
                </div>

                <div className="flex min-w-0 items-start gap-2 text-[11px] text-steel">
                ...
                <p className="break-words text-[11px] leading-5 text-steel">{chosen.note}</p>
            ...
            <Panel title="Safety protocols">
              <ol className="list-decimal space-y-2 pl-4 text-[11px] leading-5 text-steel">
        ...
        <div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:flex-wrap">
          ...
          <GhostButton onClick={() => setStep("options")} className="w-full sm:w-auto">
            Edit timing
          </GhostButton>
